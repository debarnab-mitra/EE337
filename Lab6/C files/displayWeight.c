...
void main(void)
{
	...
	while(1)												// endless 
	{
		...
		//adcVal = 0x31;
		//adcVal = adcVal/0.74;
		//adcValSum = (float)adcValSum*1.35;
		//LCD_write_weight(adcValSum);
		//adcValSum = 0;
		
			if(overflow == 10 && done == 1)
		{	done = 0;
		overflow = 0;
		adcValSum = (float)adcValSum*1.35;
		LCD_write_weight(adcValSum);
...
void LCD_write_weight(int val){
		...
		//val = val/0.74;  // conversion into grms
		fourth = val/1000 + 48;
		third = (val%1000)/100 + 48;
		second = (val%100)/10 +48;
		first = (val%10) +48;
	
		LCD_StringWrite("Weight: ",8);
		delay_ms(200);

		//LCD_write_voltage(adcVal);
		...
		LCD_DataWrite(fourth);
		delay_ms(200);
		LCD_DataWrite(third);
		delay_ms(200);
		LCD_DataWrite(second);
		delay_ms(200);
		LCD_DataWrite(first);
		delay_ms(200);
