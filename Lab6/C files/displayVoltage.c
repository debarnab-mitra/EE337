...
void LCD_write_voltage(int val);
...
void main(void)
{
	...
	while(1)												// endless 
	{
		...
		//adcVal = adcVal*4.88;
		//adcVal = 0x31;
		LCD_StringWrite(string1,9);
		delay_ms(200);

		LCD_write_voltage(adcVal);
		adcVal = 0;
		
		LCD_StringWrite(string2,3);
		delay_ms(200);
		LCD_CmdWrite(0x80);
...
void LCD_write_voltage(int val){
		...
		int fourth = val/1000 + 48;
		int third = (val%1000)/100 + 48;
		int second = (val%100)/10 +48;
		int first = (val%10) +48;
